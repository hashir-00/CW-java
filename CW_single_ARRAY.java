...
public class CW_single_ARRAY {
    ...
    //static allows me to access the arrays anywhere inside the class without providing parameters
    ...
    static ArrayList<Ticket> ticket_list_sorted= new ArrayList<>();//to copy the original list and reduce errors


    public static void main(String[] args) {

        Scanner scan = new Scanner(System.in);//defining a scanner object for input purpose
        ticket_list_sorted=ticket_list;
        ...
        while (input != '0') {
            ...
            //making sure the input is an integer(would take the previous input value ,so not implemented properly)
            ...
            do {
                try {
                    ...
                    scan.nextLine();//disregard the next line to continue
            ...
            if (input == 1) {
                ...
                while (x != 0) {
                    ...
                    //checking whether the ticket was formed successfully
    ...
    public static void buy_ticket(char[] row,int price,int row_number,ArrayList<Ticket> ticket_list) {
        ...
        char temp = 'X';//a value to store purchased seats
        ...
        {
            if (row[snum-1]=='X'){
            System.out.println("this seat is purchased");
        ...
        //checking whether the seat is empty
    ...
    public static void cancel_s(char[] row, ArrayList<Ticket> ticket_list) {
        ...
        if(snum<=0 || snum>row.length){
            System.out.println("please enter a valid seat number");}
        else {//checking if purchased or not before cancelling
            if (row[snum - 1] != 'X') {
                ...
                //methods i tried to remove a ticket
                //1st method`
    ...
    public static void sort_tickets() {
            ...
            Ticket temp;//created a temporary variable of Ticket class to hold the value of Ticket object in array list
            boolean swap=true;//control variable

           //looping until the end of the list element
            while(swap){
                swap=false;
            //here i+1 will control the error of not existing index due to i++
        for (int i=0;i+1<ticket_list_sorted.size();i++)
        {
                    //using the get method to access the element
                if (ticket_list_sorted.get(i).price>ticket_list_sorted.get(i+1).price){

                   temp= ticket_list_sorted.get(i);
                    ticket_list_sorted.set(i,ticket_list_sorted.get(i+1));
                    ticket_list_sorted.set(i+1,temp);
                    swap=true;
                    //swapping process
        ...
        for (Ticket l : ticket_list_sorted) {
            l.print();
        }
